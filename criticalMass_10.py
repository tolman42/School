...
with open("cm_10_Total.out",'w') as total_output_file:
    for n in range(len(name_array)):
        file_name = "cm_10_%s"%(name_array[n])
        
        lower_bound = 5.000000
        upper_bound = 11.000000
        lower_tolerance = 0.9990
        upper_tolerance = 1.0010
            ...
            with open('%s.inp'%(file_name),'r') as input_file, open('temp.inp','w') as output_file:
                ...
                for line in input_file:
                    if count == 26:
                        ...
                        output_file.write("10     SPH  %.6f    $Uranium-235 sphere\n"%(input_radius))
                    elif count == 41:
